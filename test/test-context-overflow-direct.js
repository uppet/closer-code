#!/usr/bin/env node
/**
 * 测试 Context Size 溢出（直接注入历史消息）
 *
 * 目的：通过直接注入大量历史消息触发 API 的 context size 限制
 * 记录错误消息和具体的 token 数值
 */
...
async function runTest() {
  ...
  console.log('Context Size 溢出测试（直接注入历史消息）');
  console.log('='.repeat(80));
  console.log('');
  ...
  try {
    ...
    for (let batch = 0; batch < Math.ceil(messageCount / batchSize); batch++) {
      ...
      try {
        ...
        console.log(`      消息：${errorMessage.substring(0, 200)}${errorMessage.length > 200 ? '...' : ''}`);
