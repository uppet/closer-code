#!/usr/bin/env node
/**
 * 测试 Context Size 溢出（使用 Ollama Provider）
 *
...
// 使用用户的自定义配置（保持原始的 ollama provider）
...
async function runTest() {
  ...
  console.log('Context Size 溢出测试（使用 Ollama Provider）');
  console.log('='.repeat(80));
  console.log('');
    ...
    for (let batch = 0; batch < Math.ceil(messageCount / batchSize); batch++) {
      ...
      try {
        ...
        const isConnectionError =
          ...
          errorMessage.toLowerCase().includes('serve');
        ...
        if (isContextOverflow) {
          ...
          console.log('   可能的原因：');
          console.log('   1. Ollama 服务未启动');
          console.log('   2. Ollama 服务地址配置错误');
          console.log('   3. 网络连接问题');
          console.log('');
          console.log('   请检查：');
          console.log(`   - Ollama 是否运行：ollama list`);
          console.log(`   - Ollama 地址：${config.ai.ollama.baseURL}`);
          console.log(`   - 模型是否下载：ollama list | grep ${config.ai.ollama.model}`);
          break;
        } else {
      ...
      if (overflowError || lastError?.message?.toLowerCase().includes('connection')) {
        break;
      }
    }
