...
- `scenarios\output\math-utils.js`
- `scenarios\output\math-utils.test.js`
- `scenarios\output\run-tests.js`
- `scenarios\run-all-scenarios.js`
- `scenarios\scenario1-batch-converter.js`
- `scenarios\scenario2-code-analyzer.js`
- `scenarios\scenario3-doc-generator.js`
- `scenarios\scenario4-log-analyzer.js`
- `scenarios\scenario5-tdd-helper.js`
- `src\ai-client.js`
- `src\bash-runner.js`
- `src\batch-cli.js`
- `src\config.js`
...
```bash
npm run build:cli && npm run build:bash && npm run build:batch
```

#### npm run build:cli

```bash
esbuild src/closer-cli.jsx --bundle --platform=node --format=esm --outfile=dist/closer-cli.js --external:ink --external:react --external:ink-text-input --external:ink-select-input --external:ansi-escapes --external:chalk --external:glob
```

#### npm run build:bash

```bash
esbuild src/bash-runner.js --bundle --platform=node --format=esm --outfile=dist/bash-runner.js
```

#### npm run build:batch

```bash
esbuild src/batch-cli.js --bundle --platform=node --format=esm --outfile=dist/batch-cli.js --external:glob
```

#### npm run start

```bash
npm run build && node dist/closer-cli.js
```
...
```bash
npm run build:batch && node dist/batch-cli.js
```
